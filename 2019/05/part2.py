...
while pc < len(intcode):
  ...
  if opcode == 1:
    ...
    if get_param(intcode, inst[2], intcode[pc + 1]):
      pc = get_param(intcode, inst[1], intcode[pc + 2])
    else:
      pc += 3
  elif opcode == 6:
    if not get_param(intcode, inst[2], intcode[pc + 1]):
      pc = get_param(intcode, inst[1], intcode[pc + 2])
    else:
      pc += 3
  elif opcode == 7:
    intcode[intcode[pc + 3]] = int(get_param(intcode, inst[2], intcode[pc + 1]) < get_param(intcode, inst[1], intcode[pc + 2]))
    pc += 4
  elif opcode == 8:
    intcode[intcode[pc + 3]] = int(get_param(intcode, inst[2], intcode[pc + 1]) == get_param(intcode, inst[1], intcode[pc + 2]))
    pc += 4
